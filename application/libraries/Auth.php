...
class Auth
{
    ...
    */
     ...
     */
    public function login($request, $redirectUrl)
    {
        if ($this->validate($request)) {
            $this->user = $this->credentials($this->userName, $this->password, $request['is_admin']);
            if ($this->user) {
                return $this->setUser($redirectUrl);
            } else {
     ...
     * @return mixed
     */
    protected function credentials($username, $password, $is_admin)
    {
        $user = $this->CI->db->get_where("users", array("username" => $username, "status" => 1, "deleted_at" => null, "is_admin" => $is_admin))->row(0);
     ...
     */
    protected function setUser($redirectUrl)
    {
    ...
    public function route_access()
    {
        ...
        if( in_array($_SERVER['HTTP_HOST'], $this->CI->config->item('localhost'))){
